...
async function ghFetch(url, token, options = {}) {
  ...
  if (!res.ok) {
    let msg = `GitHub API error (${res.status}) [${options.method || "GET"} ${url}]`;
    try {
...
export async function deleteFolder(token, owner, repo, folderPath, message, branch) {
  // Contents API يرجع الفولدر كقائمة ويعطي SHA الصحيح لكل ملف.
  async function remove(path) {
    const entries = await listContents(token, owner, repo, path, branch);
    let deleted = 0;
    for (const entry of entries) {
      if (entry.type === "dir") {
        deleted += await remove(entry.path);
      } else if (entry.type === "file") {
        await deleteFile(token, owner, repo, entry.path, message, branch, entry.sha);
        deleted += 1;
      }
    }
    return deleted;
  }
  const deleted = await remove(folderPath.replace(/^\/+|\/+$/g, ""));
  if (!deleted) throw new Error("الفولدر فاضي أو المسار غير موجود على GitHub");
  return { deleted };
}
