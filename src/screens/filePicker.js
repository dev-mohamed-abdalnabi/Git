...
export async function pickFolderRecursive(onProgress) {
  ...
  return pickFolderRecursiveFromUri(rootUri, onProgress);
}

export async function pickFolderRecursiveFromUri(rootUri, onProgress) {
  const results = [];
  // اسم فولدر SAF الجذر مثل primary:gitmobile ليس مسارًا داخل GitHub.
  // نبدأ من داخله مباشرة حتى تصبح الملفات relative/path صحيحة.
  await walk(rootUri, "", results, onProgress);
...
async function walk(uri, relativePrefix, results, onProgress) {
  ...
  for (const childUri of children) {
    ...
    const childRelativePath = relativePrefix ? `${relativePrefix}/${name}` : name;
